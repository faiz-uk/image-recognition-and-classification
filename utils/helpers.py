...
def load_model(model_path: str, 
               ...
               custom_objects: Optional[Dict[str, Any]] = None) -> keras.Model:
    """Load saved model"""
    model_path = Path(model_path)
    
    if weights_only:
        architecture_path = model_path.parent / f"{model_path.stem.replace('_weights', '')}_architecture.json"
...
def save_training_history(history: keras.callbacks.History,
                         model_name: str,
                         save_dir: Optional[Path] = None) -> str:
...
def calculate_model_size(model: keras.Model) -> Dict[str, Union[int, float]]:
    ...
    logger.info(f"Model size calculated: {total_params:,} parameters ({model_size_mb:.2f} MB)")
    return size_info
...
def create_experiment_config(model_name: str,
                           dataset_name: str,
                           **kwargs) -> Dict[str, Any]:
    ...
    if include_plots and hasattr(history, 'history'):
        try:
            ...
            if 'val_loss' in history.history:
                ...
                final_metrics['Final Validation Accuracy'] = history.history['val_accuracy'][-1]
            ...
            plot_path = plots_dir / f"{model_name}_{dataset_name}_{timestamp}_training_curves.png"
