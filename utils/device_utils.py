...
def setup_gpu(memory_growth: bool = True, mixed_precision: bool = True) -> Dict[str, Any]:
...
def get_optimal_batch_size(model_input_shape: tuple, 
    ...
    if model_name and model_name.lower() in model_complexity_map:
        ...
        logger.info(f"Using model-specific complexity for {model_name}: {model_complexity}")
    ...
    if available_memory_gb is None:
        ...
        if memory_info['gpu_available'] and memory_info['total_memory'] != "Unknown":
            available_memory_gb = memory_info['free_memory'] / (1024**3)  # Convert to GB
        else:
    ...
    if model_name and model_name.lower() in model_batch_adjustments:
        ...
        logger.info(f"Applied model-specific adjustment for {model_name}: {model_adjustment}")
...
if __name__ == "__main__":
    ...
    for model_name in ['baseline_cnn', 'mobilenet', 'resnet50', 'densenet121', 'inceptionv3']:
