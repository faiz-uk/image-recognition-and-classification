...
class MobileNetModel(BaseModel):
    ...
    def __init__(self, 
        ...
        logger.info(f"MobileNet model initialized for {num_classes} classes (alpha={alpha})")
    ...
    def build_model(self) -> Model:
        ...
        if self.input_shape[2] == 1:
            logger.info(f"Converting grayscale input {self.input_shape} to 3-channel for MobileNet compatibility")
            ...
            if self.input_shape[0] < 32 or self.input_shape[1] < 32:
                x = layers.Lambda(lambda x: tf.image.resize(x, [32, 32]), name='resize_input')(x)
            ...
            logger.info(f"Input shape {self.input_shape} too small for MobileNet, will resize internally")
            # Create input layer for small images
            inputs = keras.Input(shape=self.input_shape, name='input')
            # Resize to minimum required size
            x = layers.Lambda(lambda x: tf.image.resize(x, [32, 32]), name='resize_input')(inputs)
...
def create_mobilenet(num_classes: int, 
                    input_shape: Tuple[int, int, int] = (32, 32, 3),
                    weights: str = 'imagenet',
                    alpha: float = 1.0) -> MobileNetModel:
    """
...
def create_mobilenet_light(num_classes: int, 
                          input_shape: Tuple[int, int, int] = (32, 32, 3)) -> MobileNetModel:
    """Create lightweight MobileNet (alpha=0.5)"""
    return create_mobilenet(num_classes, input_shape, alpha=0.5)


def create_mobilenet_standard(num_classes: int, 
                             input_shape: Tuple[int, int, int] = (32, 32, 3)) -> MobileNetModel:
