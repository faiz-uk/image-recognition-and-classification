...
class BaseModel(ABC):
    ...
    def save_model_with_metadata(
        self, save_dir: Path, save_weights_only: bool = False
    ) -> str:
        """
    ...
    def fine_tune(
        self, unfreeze_layers: int = None, fine_tune_lr: float = None
    ) -> None:
        """
        ...
        if unfreeze_layers is None:
            ...
            for layer in self.base_model.layers[-unfreeze_layers:]:
                layer.trainable = True
            logger.info(
                f"Top {unfreeze_layers} base model layers unfrozen for fine-tuning"
            )
