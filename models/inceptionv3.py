...
class InceptionV3Model(BaseModel):
    ...
    def build_model(self) -> Model:
        ...
        if self.input_shape[2] == 1:
            logger.info(f"Converting grayscale input {self.input_shape} to 3-channel for InceptionV3 compatibility")
            ...
            if self.input_shape[0] < 75 or self.input_shape[1] < 75:
                x = layers.Lambda(lambda x: tf.image.resize(x, [75, 75]), name='resize_input')(x)
            ...
            logger.info(f"Input shape {self.input_shape} too small for InceptionV3, will resize internally")
            inputs = keras.Input(shape=self.input_shape, name='input')
            x = layers.Lambda(lambda x: tf.image.resize(x, [75, 75]), name='resize_input')(inputs)
            base_model = InceptionV3(weights=self.weights, include_top=False, input_tensor=x, pooling=None)
        else:
...
def create_inceptionv3(num_classes: int, 
                      input_shape: Tuple[int, int, int] = (32, 32, 3),
                      weights: str = 'imagenet') -> InceptionV3Model:
    """
