...
class BaselineCNNModel(BaseModel):
    ...
    def __init__(self, 
        ...
        self.model_info.update({
            ...
            'depth': len(conv_filters) * 2 + len(dense_units) + 1,  # Conv blocks + Dense layers + Output
        ...
        logger.info(f"Architecture: {len(conv_filters)} conv blocks + {len(dense_units)} dense layers")
...
def create_baseline_cnn(num_classes: int, 
                       input_shape: Tuple[int, int, int] = (32, 32, 3),
                       architecture: str = 'small') -> BaselineCNNModel:
    """
