...
def load_fashion_mnist_from_tar(datasets_dir: Path) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
    ...
    logger.info(f"Fashion-MNIST loaded from tar.gz: Train {X_train.shape}, Test {X_test.shape}")
    return (X_train, y_train), (X_test, y_test)


def load_fashion_mnist_from_local(datasets_dir: Path) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
    """
    ...
    for path in [train_images_path, train_labels_path, test_images_path, test_labels_path]:
        if not path.exists():
            ...
            if not plain_path.exists():
                raise FileNotFoundError(f"Fashion-MNIST file not found: {path} or {plain_path}")
    ...
    # Load training images
    train_images_path = train_images_path if train_images_path.exists() else Path(str(train_images_path).replace('.gz', ''))
    ...
    # Load training labels
    train_labels_path = train_labels_path if train_labels_path.exists() else Path(str(train_labels_path).replace('.gz', ''))
    data = read_idx_file(train_labels_path)
    magic, num_labels = struct.unpack('>II', data[:8])
    y_train = np.frombuffer(data[8:], dtype=np.uint8)
    
    # Load test images
    test_images_path = test_images_path if test_images_path.exists() else Path(str(test_images_path).replace('.gz', ''))
    data = read_idx_file(test_images_path)
    magic, num_images, rows, cols = struct.unpack('>IIII', data[:16])
    X_test = np.frombuffer(data[16:], dtype=np.uint8).reshape(num_images, rows, cols)
    
    # Load test labels
    test_labels_path = test_labels_path if test_labels_path.exists() else Path(str(test_labels_path).replace('.gz', ''))
...
def load_fashion_mnist_from_tensorflow() -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
    """
    ...
    logger.info(f"Fashion-MNIST loaded from TensorFlow: Train {X_train.shape}, Test {X_test.shape}")
    return (X_train, y_train), (X_test, y_test)
...
class FashionMNISTLoader(DataLoader):
    ...
    def __init__(self, validation_split: float = 0.2, use_local: bool = True, 
                 prefer_tar: bool = True, resize_to: Tuple[int, int] = None):
        """
        ...
        logger.info(f"Fashion-MNIST specific settings: local={use_local}, tar={prefer_tar}, resize={resize_to}")
            ...
            if self.use_local:
                if self.prefer_tar:
                    ...
                    try:
                        ...
                        logger.info("tar.gz not found, falling back to individual files...")
                        (X_train_full, y_train_full), (X_test, y_test) = load_fashion_mnist_from_local(DATASETS_DIR)
                else:
                ...
                (X_train_full, y_train_full), (X_test, y_test) = load_fashion_mnist_from_tensorflow()
...
def create_fashion_mnist_data_generators(X_train: np.ndarray, y_train: np.ndarray,
...
if __name__ == "__main__":
    ...
    try:
        ...
        train_ds, val_ds = create_fashion_mnist_data_generators(X_train, y_train, X_val, y_val, batch_size=32)
    ...
    try:
        ...
        X_train, X_val, X_test, y_train, y_val, y_test = fashion_loader_tf.load_and_preprocess()
        
        # Print dataset info
        info = fashion_loader_tf.get_data_info(X_train, X_val, X_test, y_train, y_val, y_test)
    ...
    try:
        ...
        X_train, X_val, X_test, y_train, y_val, y_test = fashion_loader_resized.load_and_preprocess()
        
        # Print dataset info
        info = fashion_loader_resized.get_data_info(X_train, X_val, X_test, y_train, y_val, y_test)
