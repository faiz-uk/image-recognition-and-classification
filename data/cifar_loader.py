...
def load_cifar10_from_local(datasets_dir: Path) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
    """
...
def load_cifar100_from_local(datasets_dir: Path, label_mode: str = 'fine') -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray]]:
    """
...
class DataLoader:
    ...
    def standardize_data(self, X_train: np.ndarray, X_val: np.ndarray, 
                        X_test: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def encode_labels(self, y_train: np.ndarray, y_val: np.ndarray, 
    ...
    def get_data_info(self, X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray,
        ...
        if hasattr(self, '_get_additional_info'):
            additional_info = self._get_additional_info(X_train, X_val, X_test, y_train, y_val, y_test)
    ...
    def create_data_generators(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        dataset_name = self.dataset_config.get('name', self.dataset_name.upper())
        logger.info(f"{dataset_name} data generators created with batch size {batch_size}")
...
class CIFAR10Loader(DataLoader):
    ...
    def load_and_preprocess(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, 
        ...
        (X_train_full, y_train_full), (X_test, y_test) = load_cifar10_from_local(DATASETS_DIR)
...
class CIFAR100Loader(DataLoader):
    ...
    def load_and_preprocess(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, 
        ...
        (X_train_full, y_train_full), (X_test, y_test) = load_cifar100_from_local(DATASETS_DIR, self.label_mode)
    ...
    def _get_additional_info(self, X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray,
        ...
        return {
            ...
            'effective_classes': self.num_classes
        }
    ...
    def _get_augmentation_function(self):
        ...
        def cifar100_augment(image, label):
            ...
            image = tf.image.random_flip_left_right(image)
            image = tf.image.random_brightness(image, max_delta=0.15)  # Slightly more aggressive
