...
def load_images_from_tar(
    datasets_dir: Path, image_list: List[str], image_size: Tuple[int, int] = (64, 64)
) -> np.ndarray:
    """
...
class CelebALoader(DataLoader):
    ...
    def __init__(
    ...
    ):  # Changed default to False for speed
        """
        ...
        Args:
            ...
            prefer_tar: Whether to prefer tar.gz format over directory structure (False = faster decompressed)
        """
        ...
        logger.info(
            f"CelebA specific settings: attribute='{target_attribute}', size={image_size}, tar={prefer_tar}"
        )
    ...
    def download_celeba(self) -> bool:
        ...
        logger.warning(
            "1. Images: https://drive.google.com/drive/folders/0B7EVK8r0v71pTUZsaXdaSnZBVzg"
        )
        logger.warning(
            "2. Attributes: https://drive.google.com/drive/folders/0B7EVK8r0v71pOC0wOVZlQnFfaGs"
        )
...
"""
    ...
    def load_and_preprocess(
        ...
        """
        logger.info(
            f"Loading CelebA dataset for '{self.target_attribute}' classification..."
        )

        # Check what's available and choose fastest option
        tar_path = DATASETS_DIR / "celeba.tar.gz"
        extracted_dir = self.celeba_dir
        images_dir = self.images_dir

        logger.info(f"Checking available CelebA formats:")
        logger.info(f"Tar.gz file: {tar_path.exists()} ({tar_path})")
        logger.info(f"Extracted dir: {extracted_dir.exists()} ({extracted_dir})")
        logger.info(f"Images dir: {images_dir.exists()} ({images_dir})")

        # Prefer decompressed files for speed, unless explicitly requested tar
        use_tar = False
        if images_dir.exists() and not self.prefer_tar:
            logger.info("Using FAST decompressed files from extracted directory")
            df_attrs = self.load_attributes()
            available_images = [f"{i:06d}.jpg" for i in range(1, len(df_attrs) + 1)]
            use_tar = False
        elif self.prefer_tar and tar_path.exists():
            logger.info("Using tar.gz format (slower but requested)")
            try:
                df_attrs, available_images = load_celeba_from_tar(DATASETS_DIR)
                use_tar = True
            except Exception as e:
                logger.warning(f"Error loading from tar.gz: {e}")
                logger.info("Falling back to decompressed files...")
                df_attrs = self.load_attributes()
                available_images = [f"{i:06d}.jpg" for i in range(1, len(df_attrs) + 1)]
                use_tar = False
        elif tar_path.exists():
            logger.info("Using tar.gz format (decompressed files not found)")
            try:
                df_attrs, available_images = load_celeba_from_tar(DATASETS_DIR)
                use_tar = True
            except Exception as e:
                logger.error(f"Failed to load from tar.gz: {e}")
                raise FileNotFoundError(
                    "No CelebA data found. Please extract celeba.tar.gz or download CelebA dataset."
                )
        else:
            logger.error(" No CelebA data found!")
            logger.info("Please either:")
            logger.info("  1. Extract celeba.tar.gz to datasets/celeba/")
            logger.info("  2. Download CelebA dataset manually")
            raise FileNotFoundError("No CelebA data found")
        ...
        if self.max_samples and len(df_attrs) > self.max_samples:
            ...
            logger.info(f"Limited to {self.max_samples} samples for faster processing")
        ...
        # Load images with appropriate method
        if use_tar:
            logger.info("Loading images from tar.gz (this will take longer)...")
            images = load_images_from_tar(DATASETS_DIR, image_names, self.image_size)
        else:
            logger.info("Loading images from decompressed files (fast)...")
        ...
        # Fix for binary classification: use raw labels instead of one-hot
        if self.num_classes == 2:
            logger.info(
                "Converting one-hot labels back to binary format for binary classification"
            )
        ...
        logger.info("CelebA data preprocessing completed successfully!")
    ...
    def _get_additional_info(
        ...
        if y_train.ndim == 1:
            # Binary classification with 1D labels
            class_distribution = {
...
def create_celeba_data_generators(
    X_train: np.ndarray,
    y_train: np.ndarray,
    X_val: np.ndarray,
    y_val: np.ndarray,
...
if __name__ == "__main__":
    ...
    try:
        ...
        X_train, X_val, X_test, y_train, y_val, y_test = (
            celeba_loader.load_and_preprocess()
        )

        # Print dataset info
        info = celeba_loader.get_data_info(
            X_train, X_val, X_test, y_train, y_val, y_test
        )
        print("\nCelebA Dataset Info:")
        ...
        train_ds, val_ds = create_celeba_data_generators(
        ...
        print(
            "Note: This used dummy data. For real usage, download CelebA dataset manually."
        )
