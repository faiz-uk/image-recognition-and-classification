...
import numpy as np  # Added for comprehensive evaluation
...
def get_model_class(architecture: str):
    ...
    if architecture.lower() not in model_classes:
        raise ValueError(
            f"Unsupported architecture: {architecture}. Available: {list(model_classes.keys())}"
        )
...
def get_data_loader(dataset_name: str, **kwargs):
    ...
    if dataset_name.lower() == "cifar100":
        ...
        resize_to = (
            kwargs.get("resize_to", (32, 32))
            if kwargs.get("standardize_size", True)
            else None
        )
        return loader_class(
            validation_split=kwargs.get("validation_split", 0.2),
...
def create_model(
    architecture: str, dataset_info: dict, dataset_name: str, **model_kwargs
):
    ...
    if architecture.lower() == "baseline_cnn":
        ...
        model = create_baseline_cnn(
            num_classes=num_classes, input_shape=input_shape, architecture=arch_size
        )
    else:
    ...
    **kwargs,
    ...
    if use_optimal_config:
        try:
            ...
            logger.info(
                f"Expected performance: {expected_perf['accuracy']:.1%} accuracy"
            )
            ...
            logger.warning(
                f"Could not get optimal configuration: {e}. Using provided/default values."
            )
    ...
    logger.info(
        f"Final parameters: epochs={epochs}, batch_size={batch_size}, lr={learning_rate}"
    )
    ...
    y_test_pred_proba = keras_model.predict(X_test, verbose=0)
    y_val_pred_proba = keras_model.predict(X_val, verbose=0)
    ...
    val_eval_results = evaluator.evaluate_model(
        ...
        model_name=f"{architecture}_validation",
        dataset_name=dataset,
    )
    ...
    test_accuracy = test_metrics.get(
        "accuracy", test_metrics.get("compile_metrics", 0.0)
    )
    ...
    if save_results:
        experiment_results = {
            ...
            "model_config": (
                model.get_model_config() if hasattr(model, "get_model_config") else {}
            ),
            "training_params": {
                "epochs": epochs,
                "batch_size": batch_size,
            ...
            # Enhanced with comprehensive evaluation results
            "comprehensive_evaluation": {
                "test_results": test_eval_results,
                "validation_results": val_eval_results,
                "evaluation_strategy": {
                    "accuracy": "Overall classification performance",
                    "precision_recall_f1": "Multi-class and imbalanced task analysis",
                    "confusion_matrix": "Class-specific performance inspection",
                    "learning_curves": "Learning dynamics tracking",
                    "model_comparison": "Performance comparison capability",
                },
            },
        ...
        # Save standard experiment results
    ...
    return (
        keras_model,
        {
            ...
            "loss": test_loss,
            "raw_metrics": test_metrics,
            "comprehensive_evaluation": test_eval_results,
        },
        history,
    )
...
def run_comprehensive_experiments(use_optimal_config: bool = True):
    ...
    logger.info(
        f"Starting comprehensive experiments: {len(models)} models × {len(datasets)} datasets = {len(models) * len(datasets)} combinations"
    )
            ...
            try:
                ...
                if use_optimal_config:
                    trained_model, test_metrics, history = train_model(
                        architecture=model, dataset=dataset, use_optimal_config=True
                    )
                else:
                    dataset_configs = {
                    ...
                    config = dataset_configs.get(
                        dataset, {"epochs": 50, "batch_size": 32}
                    )
                ...
                logger.info(
                    f"Completed {model} on {dataset}: Test Accuracy = {test_metrics.get('accuracy', 0.0):.4f}"
                )
    ...
    for result in results_summary:
        if "error" not in result:
            logger.info(
                f"{result['model']:12} | {result['dataset']:12} | "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive CNN Training Script with Intelligent Configuration"
    )

    parser.add_argument(
        "--model",
        type=str,
        choices=["baseline_cnn", "resnet50", "densenet121", "inceptionv3", "mobilenet"],
    ...
    parser.add_argument(
        "--epochs", type=int, help="Number of epochs (auto-optimized if not specified)"
    )
    parser.add_argument(
        "--batch-size", type=int, help="Batch size (auto-optimized if not specified)"
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
    ...
    parser.add_argument(
        "--alpha", type=float, default=1.0, help="MobileNet width multiplier"
    )
    parser.add_argument(
        "--trainable-params",
        type=str,
        choices=["all", "top_layers", "none"],
        default="top_layers",
    ...
    if args.comprehensive:
        logger.info(
            "Starting comprehensive experiments: 5 models × 4 datasets = 20 combinations"
        )
        ...
        for i, result in enumerate(valid_results, 1):
            logger.info(
                f"{i}. {result['model']:12} | Accuracy: {result['test_accuracy']:.4f} | Loss: {result['test_loss']:.4f}"
            )
        ...
        logger.error(
            "  2. Model comparison: --dataset <dataset> --comprehensive-models"
        )
        ...
        logger.error(
            "  python comprehensive_train.py --model resnet50 --dataset cifar10"
        )
        logger.error("")
        logger.error("  # All models on one dataset")
        logger.error(
            "  python comprehensive_train.py --dataset cifar10 --comprehensive-models"
        )
        logger.error("")
        ...
        logger.error(
            "  python comprehensive_train.py --model densenet121 --dataset cifar100 --epochs 50 --batch-size 16"
        )
        logger.error("")
        logger.error("  # Show configuration summary before training")
        logger.error(
            "  python comprehensive_train.py --model inceptionv3 --dataset celeba --show-config-summary"
        )
