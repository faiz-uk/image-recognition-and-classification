...
def get_data_loader(dataset_type: str, validation_split: float = 0.2, **kwargs):
    ...
    if dataset_type not in loaders:
        raise ValueError(
            f"Unsupported dataset: {dataset_type}. Available: {list(loaders.keys())}"
        )
...
def train_with_config(config_name: str, custom_overrides: dict = None):
    ...
    if custom_overrides:
        experiment_config = config_manager._apply_overrides(
            experiment_config, custom_overrides
        )
    ...
    if experiment_config.training.batch_size == "auto":
        ...
        logger.info(
            f"Auto-selected batch size for {experiment_config.model.architecture}: {optimal_batch_size}"
        )

    logger.info(f"Loading {experiment_config.dataset.name} dataset...")

    dataset_params = {"validation_split": experiment_config.training.validation_split}

    if experiment_config.dataset.dataset_type == "cifar100":
        dataset_params["label_mode"] = getattr(
            experiment_config.dataset, "label_mode", "fine"
        )
    elif experiment_config.dataset.dataset_type == "fashion_mnist":
        dataset_params["use_local"] = True
        dataset_params["resize_to"] = (
            (32, 32) if experiment_config.model.input_shape[:2] == (32, 32) else None
        )
    ...
    data_loader = get_data_loader(
        dataset_type=experiment_config.dataset.dataset_type, **dataset_params
    )
    ...
    optimizer = optimizer_map.get(
        experiment_config.training.optimizer.lower(),
    ...
    callbacks = [
        ModelCheckpoint(
            ...
            / f"{model_name_clean}_{dataset_name_clean}_{timestamp}.h5",
            save_best_only=True,
    ...
    logger.info(
        f"Starting training with configuration: {experiment_config.experiment_name}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified CNN Training Script with ConfigManager"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--batch-size", type=int, default=None, help="Override batch size for training"
    )
    parser.add_argument(
    ...
    if args.list_configs:
        ...
        for model, configs in sorted(config_groups.items()):
            ...
            for config in sorted(configs):
                ...
                logger.info(
                    f"  • {config:<35} | {experiment_config.dataset.name:<12} | {experiment_config.training.epochs:3d} epochs"
                )
    ...
    if not args.config:
        ...
        logger.error(
            "  python unified_train.py --list-configs         # See all available configs"
        )
        logger.error(
            "  python unified_train.py --show-matrix          # See model-dataset matrix"
        )
        logger.error(
            "  python unified_train.py --analyze-configs      # See detailed analysis"
        )
        logger.error("\nExamples:")
        logger.error("  python unified_train.py --config resnet50_cifar10_standard")
        logger.error(
            "  python unified_train.py --config baseline_cnn_fashion_mnist_fast"
        )
    ...
    try:
        ...
        model, history, results, config = train_with_config(
            args.config, custom_overrides
        )
